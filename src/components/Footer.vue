<template>
  <footer class="flex flex-col items-center w-full m-auto bg-gradient-to-t from-[#ece9e6] to-white" aria-label="网站页脚">
    <div
      ...
      <div class="flex flex-col justify-around items-center w-full desktop:flex-row">
        <div
          class="block flex-row justify-start items-start w-full h-full desktop:w-1/3 desktop:flex desktop:space-x-2">
          ...
          <ul class="footer-col tablet:px-[16px] laptop:px-[32px]">
            ...
            <li class="flex flex-row my-2 space-x-2.5 desktop:my-0 md:ml-5">
        ...
        <div class="flex flex-col justify-between items-start w-full desktop:w-2/3 desktop:flex-row">
          <nav class="footer-col tablet:px-[16px] laptop:px-[32px]" v-for="item in footerInfoArr" :key="item.title"
            ...
            <ul>
              <li
                class="item flex justify-between mt-0 desktop:mt-2 md:ml-5 items-center desktop:text-base text-sm cursor-pointer text-greyblack hover:text-brand leading-[30px]"
    ...
    <div class="mt-5 h-16 text-sm text-greyblack desktop:text-base">
      <p>© {{ currentYear }} thebettersites.com 保留所有权利</p>
    </div>
  </footer>
</template>
